...
export default function AuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    if (!jwtToken) { setStatus('error'); setErrorMsg('No token found. Please try again.'); return; }

    const finalize = async () => {
      try {
        const session = SessionManager.getSession();
        if (!session) throw new Error('No active session. Please start login again.');
        ...
        const zkProof = await ZkLoginService.fetchZkProof({
          jwtToken, ephemeralKeyPair, randomness: session.randomness,
          maxEpoch: parseInt(session.maxEpoch), userSalt: salt,
        });

        const address = ZkLoginService.getZkLoginAddress(jwtToken, salt, zkProof.addressSeed);

        SessionManager.saveProof({
          zkProof, jwtToken, address, userSalt: salt,
          maxEpoch: parseInt(session.maxEpoch), randomness: session.randomness,
          ephemeralPrivateKey: session.ephemeralPrivateKey, email: decodedJWT.email,
        });

        // Auto-create account on the gateway using the Sui address
      ...
      <div className="bg-surface-1 rounded-2xl border border-surface-2/60 p-10 max-w-md w-full text-center space-y-6">
        {status === 'processing' && (
          <>
            ...
            <div><h2 className="text-xl font-semibold text-zinc-100">Setting up your account</h2><p className="text-sm text-zinc-500">{statusText}</p></div>
          </>
        )}
        {status === 'success' && (
          <>
            ...
            <div><h2 className="text-xl font-semibold text-zinc-100">You&apos;re in!</h2><p className="text-sm text-zinc-500">Taking you to your dashboard...</p></div>
          </>
        )}
        {status === 'error' && (
          <>
            ...
            <div><h2 className="text-xl font-semibold text-zinc-100">Login Failed</h2><p className="text-sm text-red-400">{errorMsg}</p></div>
            <button onClick={() => router.replace('/login')} className="w-full h-12 bg-accent hover:bg-accent-hover text-white rounded-xl font-medium text-sm transition-all">Try Again</button>
          </>
        )}
      </div>
    </div>
