  ...
  async function handleCreate() {
    ...
    try {
      const res = await fetch("/api/keys", {
        ...
        body: JSON.stringify({ account_id: accountId, name: name.trim(), rpm_limit: 60, daily_limit: 10000 }),
      });
    ...
    return (
      <div className="bg-surface-1 border border-emerald-500/20 rounded-xl px-5 py-4 max-w-md">
        ...
        <div className="flex items-center gap-2 mb-3">
          <code className="flex-1 bg-[#0c0c0f] rounded-lg px-3 py-2 text-xs font-mono text-emerald-300 break-all">{result.key}</code>
          <button onClick={() => { navigator.clipboard.writeText(result.key); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
            className="shrink-0 px-3 py-2 bg-surface-2 hover:bg-surface-3 text-xs text-zinc-300 rounded-lg transition-colors">
            {copied ? "Copied!" : "Copy"}
          </button>
        </div>
        <button onClick={() => { setResult(null); setOpen(false); setName(""); window.location.reload(); }} className="text-xs text-zinc-500 hover:text-zinc-300">Done</button>
      </div>
    );
  }

  if (!open) {
    return <button onClick={() => setOpen(true)} className="px-4 py-2.5 bg-accent hover:bg-accent-hover text-white text-sm rounded-xl transition-colors">+ Create key</button>;
  }
