    ...
    <div className="h-full flex">
      <aside className="w-60 shrink-0 bg-surface-1 border-r border-surface-2/50 flex flex-col py-4 px-3">
        ...
        <nav className="flex flex-col gap-0.5">
          <NavLink href="/" label="Overview" current={pathname} />
          <NavLink href="/keys" label="API Keys" current={pathname} />
          <NavLink href="/usage" label="Usage" current={pathname} />
          <NavLink href="/models" label="Models" current={pathname} />
        </nav>
        ...
        <div className="mt-auto pt-3 border-t border-surface-2/50 space-y-2">
          <div className="px-3 py-2">
            ...
            <p className="text-[10px] text-zinc-600 font-mono truncate" title={proof.address}>
              {proof.address.slice(0, 8)}...{proof.address.slice(-6)}
            </p>
          </div>
          <button
            onClick={() => { SessionManager.clearAll(); router.replace('/login'); }}
  ...
  return (
    <Link href={href}
      className={`px-3 py-2.5 rounded-lg text-sm transition-colors ${
        isActive ? 'bg-surface-2 text-white' : 'text-zinc-400 hover:text-white hover:bg-surface-2'
      }`}>
      {label}
    </Link>
  );
