...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (SessionManager.getProof()) router.replace('/');
  }, [router]);
    ...
    <div className="min-h-screen bg-surface flex items-center justify-center p-6">
      <div className="w-full max-w-md space-y-8">
        <div className="text-center space-y-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-zinc-100">Pinaivu API</h1>
            <p className="text-sm text-zinc-500 mt-1">Developer Console</p>
          </div>
        </div>
        ...
        <div className="bg-surface-1 rounded-2xl border border-surface-2/60 p-8 space-y-6">
          <div>
            ...
            <p className="text-sm text-zinc-500 mt-1">Sign in with Google to manage your API keys and view usage.</p>
          </div>

          <button onClick={handleZkLogin} disabled={isLoading || !GOOGLE_CLIENT_ID}
            ...
            {isLoading ? (
              <><span className="w-5 h-5 rounded-full border-2 border-zinc-600 border-t-accent animate-spin" /><span>Connecting...</span></>
            ) : (
              <>
        ...
        <p className="text-center text-[11px] text-zinc-600">
          Powered by <span className="font-medium text-zinc-400">Sui Network</span> &middot; zkLogin
        </p>
      </div>
    </div>
  );
