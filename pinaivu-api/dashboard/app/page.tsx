      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              ...
              {health?.tls_cert_fingerprint && (
                <div>
                  ...
                  <p className="font-mono text-[11px] text-indigo-300 bg-surface-2 rounded-lg px-3 py-2 break-all">
                    {health.tls_cert_fingerprint}
                  </p>
                </div>
              )}
        ...
        <div className="bg-[#0c0c0f] border border-surface-2/60 rounded-xl overflow-hidden">
          ...
          <div className="px-5 py-4 font-mono text-[13px] leading-relaxed overflow-x-auto">
            <TermLine comment="# Install the OpenAI SDK" />
            <TermLine command="pip install openai" />
            <br />
            <TermLine comment="# Python quickstart" />
            <TermLine command="python3 << 'EOF'" />
...
print(resp.choices[0].message.content)`}</CodeBlock>
            <TermLine command="EOF" />
            <br />
            <TermLine comment="# Or use curl directly" />
            <CodeBlock>{`curl ${baseUrl}/v1/chat/completions \\
...
function InfoRow({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div>
      ...
      <p className={`text-sm text-zinc-300 ${mono ? "font-mono text-[13px]" : ""}`}>{value}</p>
    </div>
  );
}

function TermLine({ command, comment }: { command?: string; comment?: string }) {
  if (comment) return <p className="text-zinc-600">{comment}</p>;
  return <p><span className="text-emerald-400">$ </span><span className="text-emerald-300">{command}</span></p>;
}

function CodeBlock({ children }: { children: string }) {
  return <pre className="text-emerald-300/90 whitespace-pre-wrap pl-4 border-l-2 border-surface-3">{children}</pre>;
}
